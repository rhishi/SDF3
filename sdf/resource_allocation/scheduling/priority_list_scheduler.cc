 ...
 *
 *  Name            :   priority_list_scheduler.cc
 *
 ...
 *
 * $Id: priority_list_scheduler.cc,v 1.2 2008/03/06 13:59:06 sander Exp $
 *
 ...
 */
void SDFstateSpacePriorityListScheduler::TransitionSystem::State::clear()
{
 ...
 */
bool SDFstateSpacePriorityListScheduler::TransitionSystem::State::operator==(
        const State &s)
{
    if (glbClk != s.glbClk)
        return false;
 ...
 */
void SDFstateSpacePriorityListScheduler::TransitionSystem::State::print(
        ostream &out)
{
    out << "### State ###" << endl;
 ...
 */
void SDFstateSpacePriorityListScheduler::TransitionSystem::initOutputActor()
{
 ...
 */
void SDFstateSpacePriorityListScheduler::TransitionSystem
        ::initStaticOrderSchedules()
{
...
/**
 * findAllDependencies ()
 * The function performs a DFS to find all paths that go from the actor to
 * itself in the previous period (i.e. it looks for a path in the dependency
 * nodes from the last dependency node of the actor in the current period to
 * a dependency node of the same actor in the previous firing). When such a
 * path is found, the list of blocked channels is updated. Requirement is that
 * a channel must be blocked on at least one path.
 */
void SDFstateSpacePriorityListScheduler::TransitionSystem::findAllDependencies(
        bool **nodes, const int nrNodes, const int actor, const int b, 
        int *color, int *pi, int *cnt)
{
    int x;
    ...
    if (color[b])
    {
        // Is the node b the actor we are looking for?
        if (actor == b)
        {
    ...
    // Iterate over all nodes to find nodes reachable from b
    for (int a = 0; a < nrNodes; a++)
    {
        // Is node a reachable from b?
        if (nodes[b][a])
        {
            pi[a] = b;
            findAllDependencies(nodes, nrNodes, actor, a, color, pi, cnt);
            pi[a] = -1;
        }
    } 
...
/**
 * initActorPriorities ()
 * The function determines the order (priority) in which actors must be
 * checked for their start-of-firing.
 */
void SDFstateSpacePriorityListScheduler::TransitionSystem::initActorPriorities()
{
    int nrNodes, maxCnt;
    int *color, *pi, *cnt;
    bool ***edges;
    ...
    thr = stateSpaceAbstractDepGraph(bindingAwareSDFG, &nodes, &edges);
    ASSERT(thr != 0, "Graph is not deadlock free.");

    // Initialize data structures used in DFS
    nrNodes = bindingAwareSDFG->nrActors();
    color = new int [nrNodes];
    pi = new int [nrNodes];
    cnt = new int [nrNodes];
    ...
    // Find all cycles in the abstract dependency graph using a DFS
    for (int a = 0; a < nrNodes; a++)
    {
        // Visit a (no predecessor)
        color[a] = 1;

        // Iterate over all nodes reachable from a        
        for (int b = 0; b < nrNodes; b++)
            if (nodes[a][b])
                findAllDependencies(nodes, nrNodes, a, b, color, pi, cnt);

        // Remove a from the graph to avoid revisiting any cycle 
        // which contains node a.
        for (int b = 0; b < nrNodes; b++)
            nodes[a][b] = false;
    ...
    // Find the maximal number of times any actor appears in all cycles of the
    // abstract dependency graph
    maxCnt = 0;
    for (int i = 0; i < nrNodes; i++)
    {
    ...
    // Resize the vector used to store all actor priorities
    actorsOnPriority.resize(bindingAwareSDFG->nrActors());

    // Set actor priorities based on the number of times an actor appears in a 
    // cycle of the abstract dependency graph
    for (int n = maxCnt; n >= 0; n--)
    {
        for (int a = 0; a < nrNodes; a++)
        {
            if (cnt[a] == n)
            {
                actorsOnPriority.push_back(bindingAwareSDFG->getActor(a));
            }
        }
    }
 ...
 */  
TDtime SDFstateSpacePriorityListScheduler::TransitionSystem::computeThroughput(
 ...
 */
bool SDFstateSpacePriorityListScheduler::TransitionSystem::actorReadyToFire(
        SDFactor *a)
{
 ...
 * position.
 */
void SDFstateSpacePriorityListScheduler::TransitionSystem::startActorFiring(
        TimedSDFactor *a)
{
 ...
 */
bool SDFstateSpacePriorityListScheduler::TransitionSystem::actorReadyToEnd(
        SDFactor *a)
{
 ...
 */
void SDFstateSpacePriorityListScheduler::TransitionSystem::endActorFiring(
        SDFactor *a)
{
    for (SDFportsIter iter = a->portsBegin(); iter != a->portsEnd(); iter++)
    {
 ...
 */
SDFtime SDFstateSpacePriorityListScheduler::TransitionSystem::clockStep()
{
 ...
 */  
TDtime SDFstateSpacePriorityListScheduler::TransitionSystem::execSDFgraph()  
{
    ...
    while (true)  
    {
        ...
        // Start actor firings  
        for (SDFactorsIter iter = actorsOnPriority.begin();
                iter != actorsOnPriority.end(); iter++)
        {
            TimedSDFactor *a = (TimedSDFactor*)(*iter);
