 ...
 *
 * $Id: dfs.cc,v 1.2 2008/03/22 14:24:21 sander Exp $
 *
 ...
 */
static
CSDFactors getAdjacentActors(CSDFactor *a, bool transpose)
{
    CSDFactors actors;
 ...
 */
static
CSDFactor *getNextActor(CSDFactors &actors, v_int &order)
{
    CSDFactor *a = NULL;
    int orderA = -1;
 ...
 */
static
void dfsVisit(CSDFactor *u, int &time, v_int &color, v_int &d, v_int &f, 
        CSDFactor **pi, bool transpose)
{    
    // color[u] <- gray
    color[u->getId()] = 1;
    ...
    // for each v in Adj(e)
    CSDFactors adj = getAdjacentActors(u, transpose);
    for (CSDFactorsIter iter = adj.begin(); iter != adj.end(); iter++)
    {
        CSDFactor *v = *iter;
...
void dfs(CSDFgraph *g, v_int &d, v_int &f, CSDFactor **pi, bool transpose)
{
    ...
    for (CSDFactor *a = getNextActor(g->getActors(), order); a != NULL; 
