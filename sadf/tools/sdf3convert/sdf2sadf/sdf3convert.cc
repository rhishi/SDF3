...
void initSettings(int argc, char **argv)
{
    ...
    settings.xmlAppGraph = loadApplicationGraphFromFile(settings.graphFile, "sdf");	// MODULE == SDF
}
...
int main(int argc, char **argv) 
{
	...
	TimedSDFgraph* SDF = new TimedSDFgraph();
	SDF->construct(sdfNode, sdfPropertiesNode);

	// Perform the actual function

	SADF_Graph* SADF = SDF2SADF(SDF);
	...
	cout << "NOTE: SADF interprets auto-concurrency in the original SDF as absent" << endl;
